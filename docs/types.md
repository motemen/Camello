...
上の表にない**裸の名前**は、**クラス名**として読まれます（Moose と同じ読み方
です）。
...
- (TYPE-3a) 型の位置に書かれた**クォート付きの文字列は、その文字列という値**
  です。裸の名前が型を指すので、クォートは値を指すのに空いています。

  ```perl
  # Returns: 'draft' | 'live'                      Enum[draft, live]
  # Returns: MyApp::Post                           InstanceOf['MyApp::Post']
  ```

  `isa => 'MyApp::User'` がこれに巻き込まれることはありません。文字列制約は
  クォートが外れた状態で型パーサに届くので、そこに見えるのは裸の名前です。
  クォートが名前を指すのは、Type::Tiny がそう書く場所 —— `InstanceOf['Foo']`、
  `ClassName['Foo']`、`ConsumerOf['Role']`、`HasMethods['x']`、`Enum['a','b']`、
  `Dict['key' => Str]` の鍵 —— だけで、そこでは今までどおり名前として読みます。

  `'draft' | 'live'` を二つのクラスとして読んでいたときは、宣言のないクラスが
  二つできて `unknown-type` が二つ出た上に、その期待どおりの `return 'draft'`
  が `return-mismatch` になっていました。どの値を持つかは**値**の問いなので、
  `Enum` は TYPE-5e のとおりスカラーとは互いに適合します —— 拾えるのは参照を
  返したときです。
...
- 残りは次の四つのどれかです。スカラーコンテキストの型、**先頭から末尾まで
  ...
  # Returns: ()                何も返さない
  # Returns: (Value, Undef) | (Undef, Error)   どちらか一方の形
  ```
...
- (ANNOT-7e) **形そのものが選択肢であることがあります。** `(Value, Undef) |
  (Undef, Error)` は「ok か error か」のイディオムで、スロット同士が**相関して
  います** —— 片方が埋まっていればもう片方は `undef` です。これをスロットごとに
  合流させて `(Value | Undef, Error | Undef)` にすると、その形が言っている唯一の
  ことが消えます。なので合流させず、選択肢のまま持ちます。

  トップレベルの `|` で区切られた**すべての部分が先頭から末尾まで丸括弧で
  囲まれている**ときだけこの読み方をします。`Str | Undef` の `|` は型の言語の
  ものであって、この記法のものではありません。選択肢のスロット数は揃っていなければ
  ならず、揃っていなければ `bad-annotation` です。一スロットの選択肢は相関する
  相手がいないので `(Str) | (Int)` は `(Str | Int)` に畳まれます。

  `return` は**どれか一つの選択肢に合えば**通ります。個数が合わなければ、
  選択肢がいくつあっても `error` です（どの選択肢もスロット数は同じなので、
  個数は両側に書かれています）。

  推論も同じです。`return (undef, $err)` と `return ($value, undef)` を持つ
  サブルーチンは `(Undef, Error) | (Value, Undef)` になります。選択肢が
  `ListShape::ALTERNATIVES` を超えたら、そのサブルーチンが表しているのはもう
  「選択」ではないので、スロットごとの合流に畳みます。

  ただし**代入は相関を運びません**。`my ($value, $error) = f();` は `$value` に
  `Value | Undef`、`$error` に `Undef | Error` を束縛します —— 位置で読む側に
  相関を渡す仕組みがここには無いからです。選択肢が効くのは、表示されるもの
  （ホバー、シグネチャ）と、書かれたアノテーションとの照合です。
...
- (ANNOT-7a) `Returns:` とその推論された戻り値が食い違うとき、**アノテーションが
  ...
  リスト側も同じように照合されます。個数が合わなければ —— アノテーションが
  「二つ」と書き、`return` が三つ書いている ——、スロットの型が合わなければ、
  どちらもスカラー側と同じ扱いです。`return @rows` を `(Row ...)` に対して
  照合するのは要素単位です。
- (ANNOT-7e) `return-mismatch` は、両側がどれだけはっきり書かれていても
  **`warning` です**（`error` にはなりません）。相手はコメントであって、perl が
  強制するものではありません。走るのは本体の方で、食い違っているときに古いのは
  アノテーションの側であることも同じくらいあります。どちらが間違っているかを
  決めずに「二つが食い違っている」と言うのがこの診断の役目で、それだけを理由に
  ビルドを落とすべきではない、というのがこの severity です。
...
- (ANNOT-7f) 型の名前は宣言と同じ語彙に、Perl の綴りを足したものです。
  `boolean`（`Boolean` も）は `Bool`、`undef` は `Undef` で、入れ子の中でも
  同じです。このコメントを読むのは camello だけなので語彙も camello のもの
  ですが、`isa => 'boolean'` は Moose が評価する文字列で、そこでは知らない
  名前はクラス名です（TYPE-3）。
...
- (INFER-3b) **捨てられたスロットも引数です。** `my (undef, $name) = @_;` は
  invocant を受け取って捨てる書き方で、名前が一つしか出てこなくても引数は二つです。
  `undef` のスロットはシギルを持たない名前として記録され、本体の環境には何も
  束縛しません。これを読み落とすと `$name` が invocant の位置に繰り上がり、
  アリティが一つずれます（`File::DesktopEntry::lookup` がこの形です）。
- (INFER-3c) **`@_` を他所でも読んでいるなら、引数リストは `Unknown` です。**
  展開の並びより後ろに裸の `shift` / `pop` があるか、`@_` や `$_[0]` が出てくれば、
  名前で数えた個数は本当の個数ではありません。**文字列の中も見ます** —
  `"Header Error: $_[0]"` は補間スキャナ（[typecheck.md](typecheck.md) の Scopes）
  でしか見えず、`IO::Uncompress::Base` の一族はこれで一族まるごと
  「引数を取らない」と読まれていました。

- (INFER-3d) **その場で文字列にする演算子は、変数に `Str` を残します。**
  `$x =~ s/…/…/` と `$x =~ tr/…/…/`、そして `$x .= EXPR` の後、`$x` は `Str`
  です。`my $v = 何か不透明なもの; $v =~ s/^ *//; $v` は文字列を整える
  サブルーチンの書き方そのもので、演算子が何も言わないうちは丸ごと `Unknown`
  でした。

  `.=` には留保がありません。連結は必ず文字列を書き戻すからです。置換の方は
  **読み方であって保証ではありません** —— 置換は**当たったときにだけ**代入するので、
  一つもマッチしなければ元の値がそのまま残ります。それでも `s///` を書いた
  プログラムはその変数を文字列として扱っているので、INFER-2g が `Foo->new` を
  `InstanceOf['Foo']` と読むのと同じ実際主義で受け取ります。

  ただし、**`undef` でありうると分かっている側はそのまま残します**。

  ```perl
  my $s = maybe_str();     # Str|Undef
  $s =~ s/abc/def/;        # Str|Undef のまま
  ```

  `abc` が見つからなければ置換は起きず、`$s` は undef のままだからです
  （perl はそうします）。この読み方が埋めるのは**何も分かっていなかったところ**で、
  プログラムがすでに言ったことを覆しはしません（POLICY-2）。`.=` の方は
  ここでも留保なしで、undef に `.=` すると defined な文字列になります。
  なお `my $s;` は初期化子が無いだけで `Undef` ではなく `Unknown` なので、
  そこから `s///` すると `Str` です。

  言わないものが三つあります。`s/…/…/r` は書き換えた**写し**を返して変数には
  触れません（フラグの目的がそれです）。素のマッチ `$x =~ /…/` は読むだけで、
  `""` をオーバーロードしたオブジェクトはマッチさせても後もオブジェクトです。
  `chomp` は意図であって証拠ではありません —— 参照には外す末尾の区切りが無いので、
  `chomp` を通った参照は参照のままです。

  代入と同じく、記録するのは**素の変数**だけです。`$h->{k} =~ s/…/…/` は
  `$h` に対する代入ではありません。
...
- (INFER-4e) 読み取りの単位は**サイト**、つまり値がサブルーチンから出ていく場所です。
  ...
  | 本体のどこかに `goto &NAME` | `NAME` の戻り値。`goto` は末尾呼び出しで、`@_` ごと呼び先に渡り、呼び出し元が受け取るのは呼び先の答えだからです（`docs/return-inference.md`） | 同じ |
  | それ以外の綴りの `goto`（`&$code`, `&{$x}`, `$code`, ラベル） | サブルーチン全体が `Unknown` | 同じ |
...
- (METHOD-1a) `@ISA` の綴りは三つとも読みます。`our @ISA = ('Base')`、
  `@ISA = qw(Base Other)`、そして**完全修飾**の `@Foo::ISA = qw(Base)` です。
  完全修飾のものが名指しているパッケージが対象で、それは文が書かれている
  パッケージとは限りません（`@Late::ISA = (...)` を別のパッケージから書けます）。
  `qw(...)` は一つの要素に複数の名前が入っているだけで、読めない値と混同しては
  いけません — `CPAN::FTP` は `@CPAN::FTP::ISA = qw(CPAN::Debug)` と書いてあり、
  綴り二つぶんの読み落としで親を丸ごと失っていました。`@EXPORT` も同じ規則です。
...
| (DIAG-9) `return-mismatch` | warning | `Returns:` と食い違う `return` |
...
- (DIAG-9a) `return-mismatch` はこの規則には従わず、リテラルに対しても
  ...
  `return` はその無名 sub のもので、無名 sub には注釈を書く手立てがないので、
  書かれているサブルーチンの `Returns:` は何も言いません。
...
- (DIAG-7a) **`unknown-method` は既定で `info` で、世界が閉じているときだけ
  ...
  そもそも報告しません。ここで見るのはもう一段弱い条件で、**そのクラスと祖先が
  `use` しているモジュールを全部読めたか**です。モジュールは importer にサブを
  生やしますし（METHOD-6）、glob に代入することもあるので、解決できなかった `use`
  が一つでもあればメソッドの一覧は「下限」であって全体ではありません。

  名前で認識しているモジュールは穴に数えません。`use Moose` が何を生やすかは
  `Moose.pm` を読むより camello の方がよく知っているので、ファイルが検索パスに
  無くても欠けたことにはなりません（`Frameworks::note` と同じ一覧です）。プラグマも
  数えません — 何も宣言しませんし、リゾルバも読みません。

  **読めたモジュールも穴になりえます。** glob の代入先パッケージを実行時に
  決めているモジュール（`*{"${caller}::$symbol"} = ...`、`*{ $pkg . "::$name" }
  = ...`）は、読めたからこそ「そこに何が入るか列挙できない」と分かるので、
  読めたことを閉世界の根拠には数えません。生成器が呼び出し側へメソッドを渡す
  のはこの形です。

  逆に、**書いてあるパッケージ**への代入は穴ではありません。`*name` と
  `*Foo::name` はもちろん、`*{"Foo::$name"}` もそうです — 名前が計算されて
  いるのはシンボルの側で、パッケージは紙の上にあります。`Carp` は
  `*{"warnings::$_"}` を書きますが、それは `warnings` であって `use Carp` した
  誰かではありません。**名前を持てないパッケージだけが importer でありうる**
  ので、線はそこに引きます。

  **正しいクラスを見ているか**も同じだけ必要です。値に対する呼び出し
  （`$u->host`）で、そのクラスの `new` が自分以外を返すもの（INFER-2g で
  `constructs_own_class` が偽になるもの）なら `warning` には上げません。
  `URI->new` は `URI::http` を返すので `host` は `URI::_server` にあり、
  「`URI` にそんなメソッドは無い」は正しくても役に立ちません。推論はこれを
  すでに知っていて `URI->new` を `Unknown` と読みますが、`Returns: URI` や
  `isa => 'URI'` は別の道から同じクラス名に着くので、道が違うだけで本体が
  言ったことは変わらない、という扱いにします。黙るのではなく `info` なのは、
  アノテーションもまた証拠だからです。

  クラス名で名指しした呼び出し（`URI->host`）はこの限りではありません。そこで
  聞かれているのは `URI` というパッケージそのもので、`new` が何を返すかは
  関係がないからです。
- (DIAG-14a) `maybe-deref` は `warning` です。`$ary->[0]` も `$h->{k}` も
  構造的に `Maybe` なので、この診断はイディオムそのものを指すことがあり、絞り込みの
  一覧（NARROW）は定理ではないので一覧に無い書き方で守られているコードもここに
  出ます。それでも「undef かもしれない値をデリファレンスしている」は具体的なプログラムに
  ついての具体的な主張で、`unknown-method` のように読めていない世界に依存しては
  いません。
...
既定は `warning` です。`info` は「言われたいと頼んだから言われる」ものの置き場所なので、
何も頼んでいない実行がそれで埋まることはありません。`--min-severity info` が頼む形です。
`--error-on` がそれより静かな重大度を名指ししたときは、既定のほうが譲ります
（設定ファイルやフラグで書かれた `min-severity` は譲りません）。
`camello lsp` の既定は `info` のままです（`docs/lsp.md`）。
