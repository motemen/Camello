...
- (INFER-4e) 読み取りの単位は**サイト**、つまり値がサブルーチンから出ていく場所です。
  ...
  | サイト | スカラーの型 |
  | --- | --- |
  | `return EXPR` | `EXPR` をスカラーコンテキストで見た型 |
  | `return;` / `return undef` | `Undef` |
  | `return (A, B)` `return @x` `return %h` | `Unknown`（個数を `Int` と言えば `my $rows = $self->rows` を `Int` と保証することになり、それは型ではなくバグです） |
  | `return wantarray ? A : B` | `B`、つまりスカラー側の枝。ただし `B` 自身がリストなら `Unknown` |
  | `die` `croak` `confess` `throw` `exit` | サイトではありません（bottom）。join に何も足しません |
  | **末尾**: 本体の最後の文が式文であるとき | その式の型。`sub name { $_[0]->{name} }` がアクセサの書き方の半分だからです。文修飾子 (`... if $ok`) が付いた文は、条件が成り立たないときの値が条件自身の値なので `Unknown` |
  | 末尾が `if`/`unless`/`elsif`/`else` の連鎖 | 各枝の末尾の join。`else` のない連鎖は `Unknown`（偽の `if` の値は条件の値だから） |
  | 末尾がループ・素のブロック・`package`・入れ子の `sub`、空の本体 | `Unknown` |
  | 本体のどこかに `goto` | サブルーチン全体が `Unknown` |
...
- (LIMIT-6) **リストコンテキストで返る値**（INFER-4e）。`return @rows` や
  `return (A, B)` のスカラー側は `Unknown` で、リスト側はまだ読まれません。
