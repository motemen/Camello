...
### 2.3 空行の自動挿入（BLANK_LINE-1）— 当初は実装漏れだった

`sub` 定義とフェーズブロックの前後、POD と `__DATA__` の直前に空行を入れる。

**初版の本レポートはこれを見落としていた。** `declarations.pl` の差分を
アライメント規則の変更としか読んでおらず、フェーズブロック間の空行が
消えていることに気づいていなかった。差分を件数でしか見ていなかったのが原因で、
中身を読めば分かるものだった。

- `declarations.pl`, `control_flow.pl`

なお `package` / `use` 文のグループ前後の空行（BLANK_LINE-1 の3項目め、
「関連する宣言が続く場合は除く」）は未実装のままである。§4 に挙げる。
...
| （全体） | BLANK_LINE-1 の `package` / `use` グループ前後の空行が未実装 |
...
- `${^MATCH}` が `${^ MATCH}` と出力されていた（別の変数になる）。
  `^` と名前を2トークンに分けていたため、意味保存テストが構造的に
  検出できなかった。1トークンにした。
- 後置デリファレンス `->@*` の前に空白が入り、後置スライス `->@[...]` が
  未実装だった。
- BLANK_LINE-1 の実装漏れ（上記 §2.3）。
