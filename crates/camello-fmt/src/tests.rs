...
use camello_syntax::lang::TokenKind;
...
fn tokens(source: &str) -> Vec<(TokenKind, String)> {
    use camello_syntax::lang::TokenExt;
    camello_syntax::parse::parse(source)
        .syntax()
...
fn assert_preserves_comments(source: &str) {
    let comments = |text: &str| {
        use camello_syntax::lang::TokenExt;
        camello_syntax::parse::parse(text)
            .syntax()
            .descendants_with_tokens()
            .filter_map(|element| element.into_token())
...
fn input_deeper_than_the_limit_is_reported_and_kept() {
    ...
    for source in [
        ...
        let errors = &parsed.diagnostics;
        let formatted = super::format(
            &parsed.syntax(),
            &parsed.trivia,
            &FormatterOptions::default(),
        );
