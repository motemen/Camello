...
use camello_syntax::hash::OffsetMap;
...
impl<'a> Builder<'a> {
    ...
    fn mark_positions(
        ...
        let kind = camello_syntax::lang::SyntaxKind(node.kind().0).as_node();
        ...
        for child in node.children() {
            match child {
                ...
                rowan::NodeOrToken::Token(token) => {
                    let token_kind = camello_syntax::lang::SyntaxKind(token.kind().0).as_token();
                    match token_kind {
