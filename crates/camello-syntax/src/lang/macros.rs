...
macro_rules! define_language {
    (
        ...
        // `#[macro_export]` rather than a crate-private `macro_rules!`: since
        // the workspace split the callers are other crates, and a `macro_rules!`
        // is private to its own crate however it is re-exported. The export
        // lands at the crate root; the `pub use` below is what puts it beside
        // the `TokenKind` it names, which is where every caller writes it.
        #[macro_export]
        macro_rules! T {
